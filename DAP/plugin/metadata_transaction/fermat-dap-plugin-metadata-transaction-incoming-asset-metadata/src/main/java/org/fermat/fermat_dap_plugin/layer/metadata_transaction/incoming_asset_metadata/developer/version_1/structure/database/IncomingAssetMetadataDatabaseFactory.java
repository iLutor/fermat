 ...
 */
public final class IncomingAssetMetadataDatabaseFactory {

    private final PluginDatabaseSystem pluginDatabaseSystem;
    private final UUID pluginId;

    public IncomingAssetMetadataDatabaseFactory(PluginDatabaseSystem pluginDatabaseSystem, UUID pluginId) {
    ...
    public Database createDatabase() throws CantCreateDatabaseException {
        Database database;
        try {
            database = this.pluginDatabaseSystem.createDatabase(pluginId, IncomingAssetMetadataDatabaseConstants.INCOMING_ASSET_METADATA_DATABASE);
        ...
        try {
            ...
            eventsRecorderTable.addColumn(IncomingAssetMetadataDatabaseConstants.INCOMING_ASSET_METADATA_EVENTS_RECORDED_ID_COLUMN_NAME, DatabaseDataType.STRING, 255, Boolean.TRUE);
            eventsRecorderTable.addColumn(IncomingAssetMetadataDatabaseConstants.INCOMING_ASSET_METADATA_EVENTS_RECORDED_EVENT_COLUMN_NAME, DatabaseDataType.STRING, 10, Boolean.FALSE);
            eventsRecorderTable.addColumn(IncomingAssetMetadataDatabaseConstants.INCOMING_ASSET_METADATA_EVENTS_RECORDED_SOURCE_COLUMN_NAME, DatabaseDataType.STRING, 10, Boolean.FALSE);
            eventsRecorderTable.addColumn(IncomingAssetMetadataDatabaseConstants.INCOMING_ASSET_METADATA_EVENTS_RECORDED_STATUS_COLUMN_NAME, DatabaseDataType.STRING, 10, Boolean.FALSE);
            eventsRecorderTable.addColumn(IncomingAssetMetadataDatabaseConstants.INCOMING_ASSET_METADATA_EVENTS_RECORDED_TIMESTAMP_COLUMN_NAME, DatabaseDataType.LONG_INTEGER, 100, Boolean.FALSE);

            eventsRecorderTable.addIndex(IncomingAssetMetadataDatabaseConstants.INCOMING_ASSET_METADATA_EVENTS_RECORDED_TABLE_FIRST_KEY_COLUMN);

            try {
                //Create the table
                databaseFactory.createTable(pluginId, eventsRecorderTable);
            } catch (CantCreateTableException cantCreateTableException) {
                throw new CantCreateDatabaseException(CantCreateDatabaseException.DEFAULT_MESSAGE, cantCreateTableException, "Creating " + IncomingAssetMetadataDatabaseConstants.INCOMING_ASSET_METADATA_EVENTS_RECORDED_TABLE_NAME + " table", "Exception not handled by the plugin, There is a problem and I cannot create the table.");
            }
    ...
    public boolean databaseExists() {
        try {
            pluginDatabaseSystem.openDatabase(pluginId, IncomingAssetMetadataDatabaseConstants.INCOMING_ASSET_METADATA_DATABASE);
            return true;
